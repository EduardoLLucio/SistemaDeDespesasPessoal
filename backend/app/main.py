...
def cadastrar_usuario(usuario: schemas.User, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    # DEBUG: log do conteúdo recebido (remover em produção)
    try:
        logger.info("Recebido cadastro: email=%s first_name=%s", usuario.email, usuario.first_name)
        logger.info("Senha repr: %r", usuario.password)
        logger.info("Senha len chars: %d", len(usuario.password))
        logger.info("Senha len bytes UTF-8: %d", len(usuario.password.encode('utf-8')))
    except Exception:
        logger.exception("Erro ao logar dados do cadastro")

    # verificação de email existente no bd
    ...
    # Validação de senha: mínimo 8 caracteres e pelo menos uma maiúscula e uma minúscula
    ...
    # Validação do tamanho em bytes para bcrypt (máx 72 bytes)
    if len(senha.encode('utf-8')) > 72:
        raise HTTPException(status_code=400, detail="Senha muito longa. Use no máximo 72 bytes (aprox. 72 caracteres ASCII).")
...
def redefinir_senha(dados: RedefinirSenha, db: Session = Depends(get_db)):
    ...
    if len(dados.nova_senha)<8 or (dados.nova_senha.lower() == dados.nova_senha or dados.nova_senha.upper() == dados.nova_senha):
        raise HTTPException(status_code=400, detail="A senha deve ter pelo menos 8 caracteres e conter pelo menos uma letra maiúscula e uma letra minúscula")
    # usa a função segura de hash (trunca por bytes se necessário)
    usuario.hashed_password = crud.hash_password(dados.nova_senha)
    usuario.token_ativacao = None
    db.commit()
    return {"mensagem": "Senha redefinida com sucesso"}

MAIL_FROM = os.getenv("MAIL_FROM", "no-reply@example.local")

async def enviar_email(destinatario, assunto, corpo_html):
    """
    Envia via Brevo API v3 (HTTP). Requer BREVO_API_KEY e MAIL_FROM no ambiente.
    """
    api_key = os.getenv("BREVO_API_KEY")
    remetente = os.getenv("MAIL_FROM", MAIL_FROM)
    ...
    payload = {
        "sender": {"name": "Sistema de Despesas", "email": remetente},
        "to": [{"email": destinatario}],
        "subject": assunto,
        "htmlContent": corpo_html or ""
    }
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            if resp.status_code in (200, 201, 202):
                logger.info("E-mail enviado via Brevo API para %s", destinatario)
            else:
                logger.error("Falha Brevo API %s %s", resp.status_code, resp.text)
    except Exception as e:
        logger.exception("Erro ao enviar via Brevo API: %s", e)
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
